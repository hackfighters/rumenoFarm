...
const Login = ({
  ...
  const { setLoggedInUser, cart, setCart,UidData, setUidData, setfarmDtl, setLoginUserData, LoginUserData } = useContext(UserContext);

  const [viewProduct,setviewProduct]=useState(false)


  const GetAddedProduct = () => {
    // Get cart and user data from cookies
    const cartData = JSON.parse(Cookies.get("cart") ?? "[]");
    const userData = JSON.parse(Cookies.get("loginUserData") ?? "{}");

    // Get the product ID from user data
    const productId = userData && userData.pID; // Check if userData exists before accessing pID

    if (!productId) {
        // Handle the case where productId is not defined
        console.warn("Product ID is not defined in user data.");
        return;
    }

    const productsToAdd = Data.filter(product => productId.includes(product.id));
    console.warn(productsToAdd,userData && userData.pID);

    // If product exists and cart is defined
    if (productsToAdd.length > 0 && Array.isArray(cartData)) {
      // Check if any of the products already exist in the cart
      const itemsExist = productsToAdd.some(productToAdd => cartData.some(cartItem => cartItem.id === productToAdd.id));

      // If none of the products exist, add them to the cart
      if (!itemsExist) {
        ...
        console.warn(Cookies.set("cart", JSON.stringify(updatedCart)))
        console.warn(JSON.parse(Cookies.get("cart")))
      }
    }
}
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(
        // 'http://192.168.1.14:5000/rumeno_login',data
      );
      ...
      if (datastatus === "Success") {
        const { userName, FarmName, date, uID, pID, rId, sessionId } = response.data;
