...
const GoatCategoryPage = ({ }) => {
  ...
  //   product.Category && product.Category.includes("kids") ||
  //   product.name && product.name.includes("kids") ||
  //   product.Category && product.Category.includes("goat") ||
  ...
  const AddToCarts = async (item) => {
    ...
    if (loggedInUser) {
      ...
      if (!itemExists) {
        ...
        // setCart([...cart, { id: item.id, amount: 1, price: item.priceText, img: item.img, name: item.name, uID: UidData ,stock:item?.stock }]);
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" name="description" content="Worried about picky goats, weak immunity, or slow growth? Explore powerful goat supplements for all needs. Shop Now!
" />
        <title>Happy & Healthy Goats With Perfect Goat feed Supplements | Goat supplements
        </title>
          ...
          <div className="row justify-content-start">
            ...
            {uniqueItemsArray.map((item, index) => (
              <div key={index} className="col-lg-3 text-center border bg-white mx-2  my-3  shadow">
                <img src={item.img} className="w-100 mt-2" height={250} alt="Rumeno-Veterinary-Product" />
